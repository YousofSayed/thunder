...
function PostReacts({ context, setContext }) {
    ...
    return (
        <ul className={`mt-2  w-[100%] p-2 flex items-center justify-between ring-1 rounded-lg ${repost ? 'bg-white dark:bg-gray-950' : 'bg-[#eee] dark:bg-gray-900'}`}>
            <button onClick={(ev) => { doReact(ev, `love`); }} className="flex items-center gap-2">
                <i ref={reactIconRef} className={`fa-regular fa-heart cursor-pointer text-xl text-black dark:text-white ${react ? `fa-solid text-red-600 text-2xl` : null} md:hover:text-cyan-400 transition-all`}></i>
